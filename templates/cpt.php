...
//  if the get_option is true use the get_option otherwise use an empty array to store in the $options variable
			$options = get_option( 'matts_plugin_cpt' ) ?: array();
			...
			foreach ( $options as $option ) 
			{
				...
				submit_button( 'Edit', 'primary small', 'sumbit', false ); 

				echo '</form> ';

				echo '<form method="post" action="options.php" class="inline-block">';

				settings_fields( 'matts_plugin_cpt_settings' );
				echo '<input type="hidden" name="remove" value="' . $option['post_type'] . '">';
				submit_button( 'Delete', 'delete small', 'sumbit', false, array(
					'onclick' => 'return confirm("Are you sure you want to delete the Custom Post Type = (' . $option['post_type'] . ') ?\nThe data associated with it will be deleted.");'
				) ); 
		...
		<div id="tab-2" class="tab-pane <?php echo isset($_POST["edit_post"]) ? 'active' : ''  ?>">
			<form method="post" action="options.php">
				<?php
				settings_fields( 'matts_plugin_cpt_settings' );
				do_settings_sections( 'matts_cpt' );
				submit_button(); 
				?>
			</form>	
		</div>
...
function custom_post_type() {
	...
	$labels = array(
		...
		'name_admin_bar'        => __( '<?php echo $option['plural_name']; ?>', 'text_domain' ),
